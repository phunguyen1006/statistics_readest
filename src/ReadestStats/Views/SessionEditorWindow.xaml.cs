...
public partial class SessionEditorWindow : Window
{
    ...
    public SessionEditorWindow(ManualLogViewModel viewModel, ManualSessionRow? row = null)
    {
        ...
        if (row is not null) Heading.Text = "Edit manual session";
    }

    private async void Save_Click(object sender, RoutedEventArgs e)
    {
        ErrorText.Text = "";
        if (BookInput.SelectedItem is not ManualBook book) { ErrorText.Text = "Choose a physical book."; return; }
        if (DateInput.SelectedDate is not { } date || !TimeSpan.TryParse(TimeInput.Text, CultureInfo.CurrentCulture, out var time)) { ErrorText.Text = "Enter a valid date and start time."; return; }
        if (!double.TryParse(DurationInput.Text, NumberStyles.Number, CultureInfo.CurrentCulture, out var minutes) || !int.TryParse(StartPageInput.Text, out var startPage) || !int.TryParse(EndPageInput.Text, out var endPage)) { ErrorText.Text = "Duration and pages must be numbers."; return; }

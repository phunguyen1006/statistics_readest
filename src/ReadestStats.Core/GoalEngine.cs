...
public sealed class GoalEngine
{
    ...
    public GoalHistory History(GoalDefinition goal, IReadOnlyList<ReadingEvent> events, int gapMinutes, DateTimeOffset? current = null)
    {
        ...
        {
            var anchor = goal.Period switch { GoalPeriod.Daily => today.AddDays(-i), GoalPeriod.Weekly => today.AddDays(-7 * i), GoalPeriod.Monthly => today.AddMonths(-i), _ => today.AddYears(-i) }; var (start, end, _, _) = CompletedBounds(goal.Period, anchor); var value = Value(goal.Metric, events.Where(e => e.Start >= start && e.Start < end).ToArray(), gapMinutes); var hit = goal.Enabled && goal.TargetValue > 0 && value >= goal.TargetValue; hits.Add(hit); points.Add(new(anchor.ToString(goal.Period == GoalPeriod.Daily ? "MMM d" : goal.Period == GoalPeriod.Monthly ? "MMM yy" : goal.Period == GoalPeriod.Yearly ? "yyyy" : "MMM d"), hit ? 1 : 0, $"{Label(goal.Metric, value)} · {(hit ? "Goal hit" : "Not hit")}"));
        }
